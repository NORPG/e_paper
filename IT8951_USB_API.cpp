...
volatile DWORD IT8951GetDriveNo(BYTE* pDriveNo)
{
	int i;
	CString cstrDriveNo;
	CString cstrDriveName;
	BYTE  bFlag;

	cstrDriveName = "Generic Storage RamDisc 1.00";

	for(i=0;i<16;i++)
	{
		//Assign Physical Drive Name
		cstrDriveNo.Format("\\\\.\\%C:", 0x41+i); //A: B: C: D ......

		//printf("Scanning Drive %d\r\n",i);
		//Sleep(100);
		//Open
		hDev = CreateFile(
			            (LPCSTR)cstrDriveNo.GetString(),      // file name
			            ( GENERIC_READ | GENERIC_WRITE),        // access mode
                	    ( FILE_SHARE_READ | FILE_SHARE_WRITE ), // share mode
                	    NULL,                                   // SD
                	    OPEN_EXISTING,                          // how to create
		...
		if(hDev != INVALID_HANDLE_VALUE)
		{
			...
			{
				gSPTDataBuf[36] = '\0';
				if( strcmp((char*)&gSPTDataBuf[8], cstrDriveName.GetString()) == 0)
				{
					//Set Drive No.
					*pDriveNo = (BYTE)i;
...
DWORD IT8951GetSystemInfoAPI(_TRSP_SYSTEM_INFO_DATA* pstSystemInfo)
{
	...
	for(i=0;i<sizeof(_TRSP_SYSTEM_INFO_DATA)/sizeof(unsigned int);i++)
	{
	   pi[i] = SWAP_32(pi[i]);
	}
...
DWORD IT8951WriteRegAPI(DWORD ulRegAddr, DWORD ulRegVal)
{
	...
	pstSPTDBuf->Cdb[4]  = (BYTE)((ulRegAddr >> 8) & 0xFF);
	pstSPTDBuf->Cdb[5]  = (BYTE)((ulRegAddr) & 0xFF);
	pstSPTDBuf->Cdb[6]  = 0x84; //IT8951_USB_OP_WRITE_REG, IT8951WriteRegAPI
	pstSPTDBuf->Cdb[7]  = 0x00;
	pstSPTDBuf->Cdb[8]  = 0x04;
	pstSPTDBuf->Cdb[9]  = 0x00;
	pstSPTDBuf->Cdb[10] = 0x00;
...
DWORD IT8951LoadImage(BYTE* pSrcImgBuf, DWORD ulITEImageBufAddr, DWORD ulX, DWORD ulY, DWORD ulW, DWORD ulH)
{
	...
	if(ulW <= 2048 && ulW != gulPanelW)//v.1.9.4
	{
        if(gucEnHSDConvert == 1)
		{
			...
			for(j=0;j<ulH;j++)
			{
				...
				IT8951WriteMemAPI(ulCurDevImgStartBuf, (WORD)ulW, pCurStartBuf);
			}
		}
	...
	{ 
		...
		for(j=0;j<ulH;j=j+ulSendLineCnt)
		{
			...
			IT8951WriteMemAPI(ulCurDevImgStartBuf, (WORD)(ulW*ulSendLineCnt), pCurStartBuf);
...
{
	int ulBlkSize = 128*1024; //64 or 128
	int ulRemainSize = ulSize; 
	int i;
	int EraseBlkCnt;
	TSPICmdArgEraseData stArgErase;
...
DWORD IT8951SFIPageWriteAPI(TSPICmdArgData* pstSPIArg, BYTE* pWBuf)
{
	...
	pstSPTDBuf->Cdb[4]  = 0x00;
	pstSPTDBuf->Cdb[5]  = 0x00;
	pstSPTDBuf->Cdb[6]  = IT8951_USB_OP_USB_SPI_WRITE;//0x96
	pstSPTDBuf->Cdb[7]  = 0x00;
	pstSPTDBuf->Cdb[8]  = 0x00; 
	pstSPTDBuf->Cdb[9]  = 0x00;
	pstSPTDBuf->Cdb[10] = 0x00;
